// CMsgQueueEx.cpp: implementation of the CMessageQueueEx class.
//
//////////////////////////////////////////////////////////////////////
...
CMsgQueueEx::CMsgQueueEx(LPCTSTR pszName, 
						 DWORD dwQueueSize
						 )
{
	ASSERT(pszName);
...
BOOL CMsgQueueEx::Create(LPCTSTR pszName, DWORD dwQueueSize)
{
    ASSERT(pszName);
	ASSERT(pszName[0] != '\0');
	ASSERT(strlen(pszName) < MAX_NAME_LEN_EX);
...
BOOL CMsgQueueEx::PutInMessage(void *pMsg, DWORD dwSize)
{
...
BOOL  CMsgQueueEx::WaitForNormalMessage(void *pMsg, DWORD& dwBufSize, DWORD dwMilliseconds)
{
	DWORD dwWaitResult;
	HANDLE handles[2];
