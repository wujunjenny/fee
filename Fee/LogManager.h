...
{
public:
	virtual void OnWrite( char * const msg, DWORD size ) = 0;
};
...
public:
	...
	virtual void OnWrite( char * const msg, DWORD size )
	{
	/*
	FILE * fp;
	...
	fclose( fp );
		...
		*/
		...
		if(Ret)
		{
			...
			file.Write("\n", 1);
			file.Write(msg, size);
			if(file.GetLength() > 50 * 1024 * 1024)
			{
				file.Close();
...
public:
	...
	/// 当缓冲中的日志记录超过大小时，缓冲中的日志将会被清除。
	/// </param>
	LogManager( ILogWriter* lw, DWORD size );
